...
There are many more observations here for each zip code, so let's look at only one zip code to see what's going on. When joining, we took the `flood_extent` value for zip code `08008` and matched it to all observations in `indv_contrib` with the same zip code. Because `flood_extent` is specific only to zip codes and not to the individual donars that live in these areas, `indv_data$flood_extent` has many duplicate values.
